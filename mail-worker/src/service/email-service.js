...
const emailService = {
	...
	async sendBySes(c, params) {
		const sendForm = {
			FromEmailAddress: params.accountEmail,
			Destination: { ToAddresses: [...params.receiveEmail] },
			Content: {
				Simple: {
		...
		if (params.name) {
			sendForm.FromEmailAddress = `${params.name} <${params.accountEmail}>`;
		}
		if (params.text) {
			sendForm.Content.Simple.Body.Text = { Data: params.text, Charset: 'UTF-8' };
		}
		if (params.html) {
			sendForm.Content.Simple.Body.Html = { Data: params.html, Charset: 'UTF-8' };
		}
		if (!params.text && !params.html) {
			sendForm.Content.Simple.Body.Text = { Data: '', Charset: 'UTF-8' };
		}
		const rawAttachments = await this.toArrayBufferAttachments(params.attachments || []);
		if (rawAttachments.length > 0) {
			sendForm.Content.Simple.Attachments = rawAttachments.map((attachment) => ({
		...
		if (params.sendType === 'reply' && params.messageId) {
			sendForm.Content.Simple.Headers = [
				{ Name: 'In-Reply-To', Value: params.messageId },
				{ Name: 'References', Value: params.messageId },
			];
		}
